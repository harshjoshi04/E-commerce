...
export default function Category({ children }) {
  ...
  const handleCheck = (e, section, option) => {
    ...
    dispatch(fetchFilterProductApi(newFilter));
    ...
    <>
      <div className="relative w-full h-[40vh] bg-black">
        ...
        <div className="absolute text-white top-0 flex justify-center items-center h-full  w-full">
          <p className="text-4xl font-bold ">Collection's</p>
        </div>
      </div>
        ...
        <div>
              ...
              <div className="fixed inset-0 z-40 flex">
                ...
                >
                  <Dialog.Panel className="relative ml-auto flex h-full w-full max-w-xs flex-col overflow-y-auto bg-white py-4 pb-12 shadow-xl">
                    ...
                    {/* Filters */}
                    <form className="mt-4 border-t border-gray-200 transition-all">
                                  ...
                                  {section.options.map((option, optionIdx) => (
                                    ...
                                    >
                                      ...
                                      <label
                                        ...
                                        className="ml-3 min-w-0 flex-1 first-letter:uppercase text-gray-500"
                                      >
                                        {option.label}
                                      </label>
                                    </div>
          ...
          <main className="mx-auto max-w-full px-4 sm:px-6 lg:px-8">
                          ...
                          <Menu.Item key={option.name}>
                            <div
                              ...
                              className={classNames(
                                "block font-medium text-gray-900 px-4 py-2 text-sm hover:bg-gray-100 cursor-pointer"
                              )}
                              onClick={() => {
                                SortingMethod(option.name);
                              }}
                            >
                              {option.name}
                            </div>
            ...
            <section aria-labelledby="products-heading" className="pb-24 pt-6">
              ...
              <div className="grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-4 transition-all">
                {/* Filters */}
                            ...
                            {section.options.map((option, optionIdx) => (
                              ...
                              >
                                <input
                                  ...
                                  name={`${section.id}[]`}
                                  onChange={(e) => {
                                    handleCheck(e, section, option);
                                  }}
                                  defaultValue={option.value}
                                  type="radio"
                                  defaultChecked={option.checked}
                                  className="radio h-4 w-4 rounded-full accent-gray-600 border-gray-300"
                                />
                                <label
                                  htmlFor={`filter-${section.id}-${optionIdx}`}
                                  className="ml-3 text-sm text-gray-600 first-letter:uppercase"
                                >
                                  {option.label}
                                </label>
                              </div>
